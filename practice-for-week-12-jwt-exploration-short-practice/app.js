...
token = jwt.sign(
    ...
    process.env.SECRET_KEY,         // secret token from the .env file
    { expiresIn: '1h' }             // options - in this case the token will expire in one hour
);
...
// Your code here
try {
    const checkPayload = jwt.verify(token, process.env.OTHER_SECRET_KEY);
}
catch (err) {
...
setTimeout(() => {
    try {
        const checkPayload2 = jwt.verify(token2, process.env.SECRET_KEY);
    }
    catch (err) {
        console.log(err.name + ': ' + err.message);
    }
