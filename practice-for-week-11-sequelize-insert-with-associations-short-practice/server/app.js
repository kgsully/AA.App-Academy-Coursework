...
app.post('/bands/:bandId/musicians', async (req, res, next) => {
    ...
    if (firstName && lastName) {
        ...
        if (band) {
            ...
            res.status(404).json({message: "Band with specified ID not found"});
        }

    } else {
        res.status(400).json({message: "Error - First and Last names are required"})
    }
})
...
app.post('/musicians/:musicianId/instruments', async (req, res, next) => {
    ...
    if(!musician) {
        res.status(404).json({message: "Error: Musician with specified ID not found"});
    } else if (!Array.isArray(instrumentIds)) {
        res.status(400).json({message: "Error - Incorrect body format"});
    } else if (instrumentIds.length < 1) {
        res.status(400).json({message: "Error - instrumentIds array cannot be empty"});
    } else {
        musician.addInstruments(instrumentIds);
        res.status(200).json({message: `Associated ${musician.firstName} with instruments ${instrumentIds}.`});
    }
