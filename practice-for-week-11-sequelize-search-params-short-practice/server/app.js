...
app.get('/musicians', async (req, res, next) => {
    ...
    if (req.query.bandName) {
        ...
        if (req.query.bandFields) {
            if (req.query.bandFields.includes('none')) {
                includeBand.attributes = ['id'];
            } else if (!req.query.bandFields.includes('all')) {
                includeBand.attributes = req.query.bandFields;
            }
        }

        query.include.push(includeBand);
    }
   ...
   if (req.query.instrumentTypes) {
        ...
        if (req.query.instrumentFields) {
            if (req.query.instrumentFields.includes('none')) {
                includeInstruments.attributes = ['id'];
            } else if (!req.query.instrumentFields.includes('all')) {
                includeInstruments.attributes = req.query.instrumentFields;
            }
        }

        query.include.push(includeInstruments);
   };
    ...
    */
    ...
    if (req.query.order) {
        query.order = [];
        req.query.order.forEach(el => {
            query.order.push(el.split(','));
