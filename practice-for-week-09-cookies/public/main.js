...
function storeTheme(themeName) {
    ...
    // // Session Cookie:
    // let themeNameCookie = "themeName=" + themeName;
...
function restoreTheme() {
    ...
    //     setTheme(cookiesArr[1]);
    // }

    // refactored for multiple cookies
    const cookiesObj = {};
    if (cookies) {
        let cookiesArr = cookies.split("; ");
        cookiesArr.forEach(el => {
            elArr = el.split("=");
            cookiesObj[elArr[0]] = elArr[1];
        });

        if (cookiesObj.themeName) {
            setTheme(cookiesObj.themeName);
        }
    }
}
...
function clearTheme() {
    // Your code here
    document.cookie = "themeName=; max-age=-10";
}
...
function restoreName() {
    ...
    if (cookies) {
        ...
        if (cookiesObj.displayName) {
            setInputValue("display-name", cookiesObj.displayName);
        }
    }
}
...
function clearName() {
    // Your code here
    document.cookie = "displayName=; max-age=-10";
}
