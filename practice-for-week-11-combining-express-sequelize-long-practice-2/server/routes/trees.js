...
router.post('/', async (req, res, next) => {
    try {
        ...
        const retTree = await Tree.findOne({
            where: {
                tree: name
            }
        });
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        const delTree = await Tree.findByPk(req.params.id);
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const updTree = await Tree.findByPk(req.params.id);
        ...
        if (req.params.id !== id) {
            ...
            if (location) {
                updTree.location = location;
            }
            if (height) {
                updTree.heightFt = height;
            }
            if (size) {
                updTree.groundCircumferenceFt = size;
            }

            await updTree.save();

            const retTree = await Tree.findByPk(req.params.id);
...
router.get('/search/:value', async (req, res, next) => {
    ...
    trees = await Tree.findAll({
        ...
        where: {
            tree: {
                // chose to use Op.substring. Could have used [Op.like]: `%${req.params.value}%` instead
