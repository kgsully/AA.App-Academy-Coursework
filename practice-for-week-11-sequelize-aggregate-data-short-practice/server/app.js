...
app.get('/cats/:id/toys', async (req, res, next) => {
    ...
    // display `catData`
    res.json({ ...catData });   // added the ... operator in order to send only the values and not the key itself!
    // res.json({ catToysAggregateData, cat });  // ORIGINAL CODE BEFORE REFACTOR
})
...
app.get('/data-summary', async (req, res, next) => {
    ...
    summaryData.totalNumberOfCats = await Cat.count();
    summaryData.totalNumberOfToys = await Toy.count();

    summaryData.toySummary = await Toy.findAll({
        attributes: [
    ...
    summaryData.expensiveToySummary = await Toy.findAll({
        attributes: [
