...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      ...
      // Build list elements for dog names from the dogs object array.
      // This is used to replace the variable in the dogs html file to show the names list
      let dogList = "";
      dogs.forEach(dog => {
        dogList += `<li>${dog.name}</li>`;
      });

      // read in html file to text, then replace the dogsList variable with the
      // list if dog names generated earlier
    ...
    if (req.method === 'GET' && req.url === '/dogs/new') {
      ...
      // read in contents of html file containing create dog form as text
      const htmlPage = fs.readFileSync("./views/create-dog.html", "utf-8");
      const resBody = htmlPage;

      // generate response with body consisting of html page of create-dog.html
      res.statusCode = 200;
      res.setHeader("Content-Type", "text/html");
      res.write(resBody);
      return res.end();
    ...
    if (req.method === 'POST' && req.url === '/dogs') {
      // Your code here
      // generate new dog from form data and append it to the dogs array
      const newDog = {
        dogId: getNewDogId(),
        name: req.body.name,
