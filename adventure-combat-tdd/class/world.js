...
class World {
  ...
  static getPetsInRoom(room) {
    return World.pets.filter(pet => pet.currentRoom === room);
  }
  ...
  static loadWorld(worldData) {
    ...
    for (let i = 0 ; i < petList.length ; i++) {
      ...
      let petData = petList[i];
      let petRoom = World.rooms[enemyData.room];
      let newPet = new Pet(petData.name, petData.description, petRoom);
      World.pets.push(newPet);
    }
