   ...
   const params = [req.params.id];
    ...
    db.get(sql, params, (err, row) => {
        if(err) {
            ...
            if (row) {
                console.log(req.method, req.url, "- Result:", row);
                res.status(200).json(row);
            } else {
                console.log(req.method, req.url, " - Error: 404 - Requested resource not found...");
                res.status(404).json("Error: 404 - Requested resource not found...");
            }
        }
    });
});
...
app.use((err, req, res, next) => {
    console.log(err);
    res.status(404).json("Error: 404 - Can't find the requested resource...");
});
